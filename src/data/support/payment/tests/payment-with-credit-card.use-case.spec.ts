...
import { PaymentError } from 'src/domain/_layer/data/dto/payment-response.dto';
...
import { PaymentWithCreditCardDomain } from 'src/domain/support/payment/payment-with-credit-card.domain';
...
describe(PaymentWithCreditCardDomain.name, () => {
  /** - Setup - */
  const setup: TestSetup<PaymentWithCreditCardDomain> = TestSetup.run(PaymentWithCreditCardDomain);
  ...
  test('Create credit card payment', async () => {
    /** - Setup - */
    ...
    // jest mock
    const cartToken: TokenEntity = { token: '::card-token::' };
    const externalPaymentId: string = '::external::';
    jest
      .spyOn(setup.servicesMocks.paymentGatewayService, 'paymentWithCreditCard')
      .mockImplementation(async (_userId: string, _payment: PaymentDto, gotCardToken: TokenEntity) => {
        if (cartToken.token === gotCardToken.token) return Either.right(externalPaymentId);
        else return Either.left(PaymentError.UNKNOWN_PAYMENT_ERROR);
      });

    /** - Run - */
    const payment: PaymentDto = await setup.useCase
      .paymentWithCreditCard(insertedUser.id, cartToken, cart, reqParentId)
      .unsafeRun();

    /** - Test - */
    const expectedPayment: Partial<PaymentDto> = {
      creditCard: {
        token: cartToken.token,
  ...
  test('Create credit card payment with coupon percentage', async () => {
    /** - Setup - */
    const QUERY_PRICE: number = 100_00;
    const PACKAGE_PURCHASE_PRICE: number = 150_00;
    const PACKAGE_ATTRIBUTED_VALUE: number = 200_00;
