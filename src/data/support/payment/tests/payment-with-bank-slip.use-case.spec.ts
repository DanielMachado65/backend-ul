...
import { PaymentWithBankSlipDomain } from 'src/domain/support/payment/payment-with-bank-slip.domain';
...
describe(PaymentWithBankSlipDomain.name, () => {
  /** - Setup - */
  const setup: TestSetup<PaymentWithBankSlipDomain> = TestSetup.run(PaymentWithBankSlipDomain);
  ...
  test('Create bank slip payment', async () => {
    /** - Setup - */
    ...
    jest
      .spyOn(setup.servicesMocks.paymentGatewayService, 'paymentWithBankSlip')
      .mockImplementation(async () => Either.right(externalPaymentId));

    /** - Run - */
    const payment: PaymentDto = await setup.useCase.paymentWithBankSlip(insertedUser.id, cart, reqParentId).unsafeRun();
    ...
    const expectedPayment: Partial<PaymentDto> = {
      ...
      paid: false,
      type: PaymentType.BANKING_BILLET,
  ...
  test('Create bank slip payment with coupon percentage', async () => {
    /** - Setup - */
    const QUERY_PRICE: number = 100_00;
    const PACKAGE_PURCHASE_PRICE: number = 150_00;
    const PACKAGE_ATTRIBUTED_VALUE: number = 200_00;
  ...
  test("Ensure bank slip payment if user doesn't have all tenants registered", async () => {
    /** - Setup - */
    const QUERY_PRICE: number = 100_00;
    const PACKAGE_PURCHASE_PRICE: number = 150_00;
    const PACKAGE_ATTRIBUTED_VALUE: number = 200_00;
